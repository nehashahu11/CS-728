...
class VanillaRNN(nn.Module):
    ...
    def forward(self, u: torch.Tensor):
        ...
        T, B, _ = u.shape
        o_s = torch.zeros((T, B, self.nout))
        h_0 = torch.zeros((B, self.nhid))
        h_s = torch.zeros((T, B, self.nhid))

        for t in range(T):
            x_t = u[t]
            h_t = self.act(x_t@self.W_uh + h_0@self.W_hh + self.b_hh)
            h_s[t] = h_t
            h_0 = h_t


        all_logits = h_s @ self.W_hy + self.b_hy

        if self.classif_type == "softmax":
            return all_logits.reshape(T * B, self.nout), h_s
        else:
            return all_logits[-1], h_s
