...
def grad_time_profile(task, model, x: torch.Tensor, y_onehot: torch.Tensor, collect_extras: bool = False):
    ...
    # 2. Compute gradients wrt hidden states h: shape (T, B, H)
    # This returns dL/dh for all timesteps
    dh = torch.autograd.grad(loss, h, retain_graph=True)[0] # 
    # dh has shape (T, B, H). We take norm over H, then mean over B.
    g_t = torch.norm(dh, p=2, dim=2).mean(dim=1) # [cite: 41, 42, 43]
    a_t = model.act_deriv_from_h(h).mean(dim=(1, 2))
    
    # 5. sat_t: hidden distance-to-saturation
